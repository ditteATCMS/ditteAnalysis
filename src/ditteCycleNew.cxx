...
#include "include/ditteCycleNew.h"
...
void ditteCycleNew::begin_dataset(Context & ctx){
    // 1. set up selections:
    ...
    // 2. Set up Histograms:
    h_nocuts.reset(new ditteHistsNew(ctx, "NoCuts"));
    h_btag.reset(new ditteHistsNew(ctx, "BTag"));
    h_nobtag.reset(new ditteHistsNew(ctx, "NoBTag"));
    h_topsel.reset(new ditteHistsNew(ctx, "TopSel"));
    ...
    // this cycle on an output file containing these branches, e.g. one previously produced with ditteCycleNew ...
    read_example_data = string2bool(ctx.get_setting("readditteData"));
...
void ditteCycleNew::process(EventCalc & event, Context & ctx) {
    ...
    if(read_example_data){
        cout << "ditte data read: int = " << event_intdata_in << "; jet pt = " << event_jetdata_in.pt() << endl;
    }
...
// as we want to run the ditteCycleNew directly with AnalysisModuleRunner, we have to make sure we are found by the class name.
// This is achieved by calling this macro:
REGISTER_ANALYSIS_MODULE(ditteCycleNew)
