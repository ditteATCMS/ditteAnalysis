// $Id: ditteCycle.cxx,v 1.10 2012/12/07 14:21:51 peiffer Exp $

#include <iostream>

using namespace std;

#include "include/ditteCycle.h"
#include "include/ditteHists.h"
...
void ditteCycle::BeginCycle() throw( SError ) 
{
...
void ditteCycle::EndCycle() throw( SError ) 
{
...
void ditteCycle::BeginInputData( const SInputData& id ) throw( SError ) 
{
  ...
  RegisterHistCollection( new ditteHists("NoCuts") );
  RegisterHistCollection( new HypothesisHists("Chi2_NoCuts", m_chi2discr ) );

  //histograms with and without b tagging
  RegisterHistCollection( new ditteHists("BTag") );
  RegisterHistCollection( new ditteHists("NoBTag") );
  ...
  RegisterHistCollection( new ditteHists("TopSel") );
...
void ditteCycle::EndInputData( const SInputData& id ) throw( SError ) 
{
  AnalysisCycle::EndInputData( id );
}

void ditteCycle::BeginInputFile( const SInputData& id ) throw( SError ) 
{
...
void ditteCycle::ExecuteEvent( const SInputData& id, Double_t weight) throw( SError ) 
{
  ...
  // all thse BaseHists* vairables private member variables of ditteCycle and
